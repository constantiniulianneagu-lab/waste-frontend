  ...
  const fetchReports = async () => {
    try {
      ...
      if (response.success && response.data) {
        ...
        // Construire summaryData diferit pentru fiecare tab
        let summary;
        
        if (activeTab === 'recycling') {
          // Calculăm totalurile din tichete dacă backend-ul nu le returnează
          const calcDelivered = ticketsList.reduce((sum, t) => sum + (parseFloat(t.delivered_quantity_tons) || 0), 0);
          const calcAccepted = ticketsList.reduce((sum, t) => sum + (parseFloat(t.accepted_quantity_tons) || 0), 0);
          
          summary = {
            total_delivered: response.data.summary?.total_delivered_tons || response.data.summary?.total_delivered || calcDelivered,
            total_accepted: response.data.summary?.total_accepted_tons || response.data.summary?.total_accepted || calcAccepted,
            ...
            clients: groupRowsByNameWithCodes(response.data.recipients || response.data.clients || response.data.operators || []),
          };
        } else if (activeTab === 'recovery') {
          summary = {
            total_delivered: response.data.summary?.total_delivered_tons || response.data.summary?.total_delivered || 0,
            total_accepted: response.data.summary?.total_accepted_tons || response.data.summary?.total_accepted || 0,
            total_tickets: response.data.summary?.total_tickets || ticketsList.length,
            date_range: {
              from: new Date(filters.from).toLocaleDateString('ro-RO'),
              to: new Date(filters.to).toLocaleDateString('ro-RO'),
            ...
            clients: response.data.recipients || response.data.clients || response.data.operators || [],
          };
          ...
          summary = {
            total_rejected: response.data.summary?.total_rejected_tons || response.data.summary?.total_rejected || 0,
            total_tickets: response.data.summary?.total_tickets || ticketsList.length,
            date_range: {
              from: new Date(filters.from).toLocaleDateString('ro-RO'),
              to: new Date(filters.to).toLocaleDateString('ro-RO'),
