...
// Schema de culori: Rose - Instituțional Modern
...
const SortIcon = ({ field, sortConfig }) => {
  ...
  return sortConfig.dir === 'asc'
    ? <ChevronUp className="w-3.5 h-3.5 ml-1 text-rose-600" />
    : <ChevronDown className="w-3.5 h-3.5 ml-1 text-rose-600" />;
};
...
const RecoveryReportView = ({
  ...
  const [filterRecoverer, setFilterRecoverer] = useState('');
  ...
  const recovererOptions = useMemo(() => {
    if (!tickets?.length) return [];
    const unique = [...new Map(tickets.map(t => [
      t.recipient_id,
      t.recipient_name
    ]).filter(([id, name]) => id && name)).entries()];
    return unique.sort((a, b) => a[1].localeCompare(b[1]));
  }, [tickets]);

  const wasteCodeOptions = useMemo(() => {
    if (!tickets?.length) return [];
    return [...new Set(tickets.map(t => t.waste_code).filter(Boolean))].sort();
  }, [tickets]);

  const hasActiveFilters = search || filterSupplier || filterRecoverer || filterWasteCode;
  ...
  const processedTickets = useMemo(() => {
    ...
    if (filterRecoverer) {
      result = result.filter(t => String(t.recipient_id) === filterRecoverer);
    }
    ...
    if (sortConfig.key) {
      result.sort((a, b) => {
        ...
        if (sortConfig.key === 'ticket_date') {
          ...
          va = parseFloat(a.delivered_quantity_tons) || 0;
          vb = parseFloat(b.delivered_quantity_tons) || 0;
        } else if (sortConfig.key === 'accepted') {
    ...
    return result;
  }, [tickets, search, filterSupplier, filterRecoverer, filterWasteCode, sortConfig]);
  ...
  const selectClass = "h-8 px-2 text-xs bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-md text-gray-800 dark:text-gray-200 focus:ring-1 focus:ring-rose-500 focus:border-rose-500 transition-colors";
  ...
  const _recipientId = isControlled ? secFilters.recipientId : filterRecoverer;
  const _setRecipientId = isControlled ? (v) => { secFilters.setRecipientId(v); secFilters.setPage(1); } : (v) => setFilterRecoverer(v);
  ...
  const _recipientOpts = isControlled ? secFilters.operatorOptions : recovererOptions;
                ...
                return (
                  <div key={idx} className="bg-gray-50 dark:bg-gray-900/30 rounded-lg p-3 border border-gray-200 dark:border-gray-700">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="text-base font-bold text-rose-600 dark:text-rose-400 ml-2">{formatNumberRO(supplierTotal)} t</span>
                    </div>
                            ...
                            <div key={cIdx}>
                              <div className="flex items-center justify-between mb-1">
                                ...
                                <span className="text-xs font-medium">
                                  <span className="font-bold text-rose-600 dark:text-rose-400">{formatNumberRO(codeQty)} t</span>
                                  <span className="text-gray-500 dark:text-gray-400 ml-1">({codePercent}%)</span>
                                </span>
                              </div>
                              <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1.5">
                                <div className="bg-rose-500 h-1.5 rounded-full transition-all duration-300" style={{ width: `${codePercent}%` }} />
                              </div>
                            </div>
                          );
                        })}
            ...
            {(!summaryData?.operators || summaryData.operators.length === 0) ? (
              ...
              summaryData.operators.map((operator, idx) => {
                ...
                return (
                  <div key={idx} className="bg-gray-50 dark:bg-gray-900/30 rounded-lg p-3 border border-gray-200 dark:border-gray-700">
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-sm font-semibold text-gray-900 dark:text-white truncate">{operator.name}</span>
                      <span className="text-base font-bold text-rose-600 dark:text-rose-400 ml-2">{formatNumberRO(operatorTotal)} t</span>
                    </div>
                    {operator.codes?.length > 0 && (
                      <div className="space-y-2">
        ...
        <div className="px-4 py-3 border-b border-gray-200 dark:border-gray-700">
          <div className="flex items-center gap-2 flex-wrap">
            <h3 className="text-base font-semibold text-gray-900 dark:text-white whitespace-nowrap">
              Tichete Valorificare <span className="text-sm font-normal text-gray-500 dark:text-gray-400">{_totalLabel}</span>
            </h3>
            ...
            <div className="relative">
              ...
              <input type="text" value={_search} onChange={e => _setSearch(e.target.value)}
                placeholder="Nr. tichet..."
                className="h-8 pl-8 pr-3 w-[180px] text-xs bg-gray-50 dark:bg-gray-900 border border-gray-300 dark:border-gray-600 rounded-md text-gray-800 dark:text-gray-200 placeholder-gray-400 focus:ring-1 focus:ring-rose-500 focus:border-rose-500 transition-colors" />
            </div>
            ...
            <select value={_recipientId} onChange={e => _setRecipientId(e.target.value)} className={selectClass}>
              <option value="">Toți valorificatorii</option>
              {(_recipientOpts||[]).map((r,i) => <option key={i} value={isControlled ? (r.recipient_id||r.name) : r[0]}>{isControlled ? r.name : r[1]}</option>)}
            </select>
            <select value={_wasteCodeId} onChange={e => _setWasteCodeId(e.target.value)} className={selectClass}>
              ...
              {(_wasteCodeOpts||[]).map((wc,i) => <option key={i} value={isControlled ? wc.id : wc}>{isControlled ? wc.label : wc}</option>)}
            </select>
            {_hasActive && (
            ...
            {canCreate && (
              <button onClick={onCreate} className="h-8 px-4 text-sm font-medium bg-rose-600 hover:bg-rose-700 text-white rounded-lg transition-colors shadow-sm flex items-center gap-2 whitespace-nowrap">
          ...
          <table className="w-full">
            <thead className="bg-gray-50 dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700">
              <tr className="text-xs font-medium text-gray-700 dark:text-gray-300 uppercase tracking-wider">
                <th className={thSortClass} onClick={() => _handleSort('ticket_number')}>
                  <span className="flex items-center">Tichet Cântar <_SortIcon field="ticket_number" /></span>
                </th>
                <th className={thSortClass} onClick={() => _handleSort('ticket_date')}>
                  <span className="flex items-center">Data <_SortIcon field="ticket_date" /></span>
                </th>
                <th className={thClass}>Furnizor</th>
                {/* ✅ Valorificator în header — Nr. Auto în expand */}
                <th className={thClass}>Valorificator</th>
                <th className={thClass}>Cod Deșeu</th>
                <th className="px-4 py-3 text-center cursor-pointer select-none hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors" onClick={() => _handleSort('delivered')}>
                  <span className="flex items-center justify-center">Cant. Livrată <_SortIcon field="delivered" /></span>
                </th>
                <th className="px-4 py-3 text-center cursor-pointer select-none hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors" onClick={() => _handleSort('accepted')}>
                  <span className="flex items-center justify-center">Cant. Acceptată <_SortIcon field="accepted" /></span>
                </th>
                ...
                _tickets.map((ticket) => (
                  <React.Fragment key={ticket.id}>
                    <tr className="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                      ...
                      {/* ✅ Valorificator în rând */}
                      <td className="px-4 py-3 text-sm text-gray-900 dark:text-white">{ticket.recipient_name || 'N/A'}</td>
                      ...
                      <tr className="bg-gray-50 dark:bg-gray-800/30">
                        <td colSpan="9" className="px-4 py-4">
                          <div className="grid grid-cols-3 gap-4 text-sm mb-4">
                            ...
                            <div>
                              <span className="text-gray-500 dark:text-gray-400 block mb-1">Operator Valorificare:</span>
                            ...
                            <div>
                              ...
                              <p className="font-semibold text-rose-600 dark:text-rose-400">{formatNumberRO(ticket.delivered_quantity_tons)} t</p>
                            </div>
                            <div>
                              <span className="text-gray-500 dark:text-gray-400 block mb-1">Cantitate Acceptată:</span>
                              <p className="font-semibold text-rose-500 dark:text-rose-500">{formatNumberRO(ticket.accepted_quantity_tons)} t</p>
                            </div>
                            <div>
