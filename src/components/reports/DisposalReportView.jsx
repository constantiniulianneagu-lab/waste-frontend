    ...
    return (
      <div className="flex items-center justify-center py-12">
        <div className="text-center">
          <div className="w-16 h-16 border-4 border-red-600 dark:border-red-500 border-t-transparent rounded-full animate-spin mx-auto mb-4"></div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden h-[320px] flex flex-col">
          ...
          <div className="p-4 flex flex-col justify-between flex-1">
            ...
            <div className="text-center py-4"><p className="text-xs text-red-600 dark:text-red-400 mb-2">● Cantitate livrată</p><p className="text-3xl font-black text-red-700 dark:text-red-400">{formatNumberRO(delivered)} <span className="text-base">t</span></p></div>
            <div className="text-center py-2"><p className="text-xs text-rose-600 dark:text-rose-400 mb-2">● Cantitate acceptată</p><p className="text-3xl font-black text-rose-700 dark:text-rose-400">{formatNumberRO(accepted)} <span className="text-base">t</span></p></div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden h-[320px] flex flex-col">
          <div className="bg-gradient-to-br from-red-500 to-red-600 p-4">
            <div className="flex items-center gap-3 text-white">
              ...
              <div className="min-w-0"><h3 className="text-sm font-semibold">Furnizori Deșeuri</h3></div>
            </div>
          </div>
                  ...
                  return (
                    <div key={idx} className="bg-gray-50 dark:bg-gray-900/30 rounded-lg p-3 border border-gray-200 dark:border-gray-700">
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <span className="text-lg font-black text-red-600 dark:text-red-400 ml-2">{formatNumberRO(supplier.total)} t</span>
                      </div>
                      {supplier.codes && supplier.codes.length > 0 && (
                        <div className="space-y-2">
                          {supplier.codes.map((code, codeIdx) => {
                            ...
                            return (<div key={codeIdx}><div className="flex items-center justify-between mb-1"><span className="text-xs font-medium text-gray-700 dark:text-gray-300">{code.code}</span><span className="text-xs font-bold text-red-600 dark:text-red-400">{codePercentage}%</span></div><div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2"><div className="bg-gradient-to-r from-red-500 to-red-600 h-2 rounded-full transition-all duration-300" style={{width: `${codePercentage}%`}}></div></div></div>);
                          })}
                        </div>
                      )}
                    </div>
        ...
        {/* CARD 3 - OPERATOR DEPOZIT */}
        <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden h-[320px] flex flex-col">
          <div className="bg-gradient-to-br from-rose-600 to-red-700 p-4">
            <div className="flex items-center gap-3 text-white">
              <div className="w-10 h-10 bg-white/20 rounded-lg flex items-center justify-center flex-shrink-0">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" /></svg>
              </div>
              <div className="min-w-0"><h3 className="text-sm font-semibold">Operator Depozit Deșeuri</h3></div>
            </div>
          </div>
          <div className="p-4 overflow-y-auto flex-1">
                  ...
                  return (
                    <div key={idx} className="bg-gray-50 dark:bg-gray-900/30 rounded-lg p-3 border border-gray-200 dark:border-gray-700">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <span className="text-lg font-black text-rose-600 dark:text-rose-400 ml-2">{formatNumberRO(client.total)} t</span>
                      </div>
                      <div className="flex items-center justify-between mb-1"><span className="text-xs text-gray-500 dark:text-gray-400">Procent din total:</span><span className="text-xs font-bold text-rose-600 dark:text-rose-400">{percentage}%</span></div>
                      <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2"><div className="bg-gradient-to-r from-rose-500 to-red-500 h-2 rounded-full transition-all duration-300" style={{width: `${percentage}%`}}></div></div>
                      {client.codes && client.codes.length > 0 && (<div className="mt-2 flex flex-wrap gap-1">{client.codes.map((code, cIdx) => (<span key={cIdx} className="text-xs px-2 py-0.5 bg-rose-100 dark:bg-rose-900/30 text-rose-700 dark:text-rose-300 rounded">{code.code}</span>))}</div>)}
                    </div>
                  );
                })}
              </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
        <div className="p-4 border-b border-gray-200 dark:border-gray-700">
          <div className="flex items-center justify-between">
            <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Tichete Eliminare ({pagination?.total || 0})</h3>
            <div className="flex gap-3">
              <button onClick={onCreate} className="px-4 py-2 text-sm font-medium bg-gradient-to-br from-red-500 to-red-600 hover:from-red-600 hover:to-red-700 text-white rounded-lg transition-all duration-200 shadow-md flex items-center gap-2">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" /></svg>
                Adaugă tichet
              </button>
              <button onClick={onExport} disabled={exporting || !tickets || tickets.length === 0} className="px-4 py-2 text-sm font-medium bg-gradient-to-br from-emerald-500 to-emerald-600 hover:from-emerald-600 hover:to-emerald-700 text-white rounded-lg transition-all duration-200 shadow-md flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed">
                {exporting ? (<><div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />Se exportă...</>) : (<><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>Export date</>)}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead>
              <tr className="bg-gradient-to-r from-red-500/10 to-rose-600/10 dark:from-red-600/20 dark:to-rose-700/20 border-b border-gray-200 dark:border-gray-700">
                <th className="px-4 py-3 text-left text-xs font-bold text-gray-700 dark:text-gray-300 uppercase">Număr Tichet</th>
                ...
                <th className="px-4 py-3 text-left text-xs font-bold text-gray-700 dark:text-gray-300 uppercase">Nr. Auto</th>
                <th className="px-4 py-3 text-left text-xs font-bold text-red-700 dark:text-red-400 uppercase">Cant. Livrată</th>
                <th className="px-4 py-3 text-left text-xs font-bold text-rose-700 dark:text-rose-400 uppercase">Cant. Acceptată</th>
                ...
                tickets.map((ticket) => (
                  <React.Fragment key={ticket.id}>
                    <tr className="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                      ...
                      <td className="px-4 py-3 text-sm font-bold text-red-700 dark:text-red-400">{formatNumberRO(ticket.delivered_quantity_tons)} t</td>
                      <td className="px-4 py-3 text-sm font-bold text-rose-700 dark:text-rose-400">{formatNumberRO(ticket.accepted_quantity_tons)} t</td>
                      ...
                      <tr className="bg-gray-50 dark:bg-gray-800/30">
                        <td colSpan="9" className="px-4 py-4">
                          <div className="grid grid-cols-3 gap-4 text-sm mb-4">
                            ...
                            <div><span className="text-gray-500 dark:text-gray-400 block mb-1">Operator Depozit:</span><p className="font-medium text-gray-900 dark:text-white">{ticket.recipient_name}</p></div>
                            ...
                            <div><span className="text-red-600 dark:text-red-400 block mb-1">Livrată:</span><p className="font-bold text-red-700 dark:text-red-400">{formatNumberRO(ticket.delivered_quantity_tons)} t</p></div>
                            <div><span className="text-rose-600 dark:text-rose-400 block mb-1">Acceptată:</span><p className="font-bold text-rose-700 dark:text-rose-400">{formatNumberRO(ticket.accepted_quantity_tons)} t</p></div>
                            <div><span className="text-orange-600 dark:text-orange-400 block mb-1">Diferență:</span><p className="font-bold text-orange-700 dark:text-orange-400">{formatNumberRO(ticket.difference_tons)} t ({ticket.delivered_quantity_tons > 0 ? ((ticket.difference_tons / ticket.delivered_quantity_tons) * 100).toFixed(2) : 0}%)</p></div>
                          </div>
                          <div className="flex gap-2">
                            <button onClick={() => onEdit(ticket)} className="px-3 py-1.5 text-xs font-medium bg-gradient-to-br from-red-500 to-red-600 hover:from-red-600 hover:to-red-700 text-white rounded shadow-md">Editează</button>
                            <button onClick={() => onDelete(ticket.id)} className="px-3 py-1.5 text-xs font-medium bg-gradient-to-br from-gray-500 to-gray-600 hover:from-gray-600 hover:to-gray-700 text-white rounded shadow-md">Șterge</button>
                          </div>
                        </td>
                      </tr>
                    )}
