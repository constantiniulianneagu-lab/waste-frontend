...
const RecyclingReportView = ({
  ...
  const recyclerOptions = useMemo(() => {
    ...
    const unique = [...new Map(tickets.map(t => [
      t.recipient_id || t.client_id,
      t.client_name || t.recipient_name
  ...
  const wasteCodeOptions = useMemo(() => {
    if (!tickets?.length) return [];
    const unique = [...new Set(tickets.map(t => t.waste_code).filter(Boolean))].sort();
    return unique;
  }, [tickets]);

  const hasActiveFilters = search || filterSupplier || filterRecycler || filterWasteCode;
  ...
  const processedTickets = useMemo(() => {
    ...
    if (filterRecycler) {
      result = result.filter(t =>
        String(t.recipient_id || t.client_id) === filterRecycler
      );
    }
    ...
    return result;
  }, [tickets, search, filterSupplier, filterRecycler, filterWasteCode, sortConfig]);
  ...
  // ── Stiluri ────────────────────────────────────────────────────────────
  const selectClass = "h-8 px-2 text-xs bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-md text-gray-800 dark:text-gray-200 focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500 transition-colors";
  ...
  const _recipientId = isControlled ? secFilters.recipientId : filterRecycler;
  const _setRecipientId = isControlled ? (v) => { secFilters.setRecipientId(v); secFilters.setPage(1); } : (v) => setFilterRecycler(v);
  ...
  const _recipientOpts = isControlled ? secFilters.operatorOptions : recyclerOptions;
                ...
                return (
                  <div key={idx} className="bg-gray-50 dark:bg-gray-900/30 rounded-lg p-3 border border-gray-200 dark:border-gray-700">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="text-base font-bold text-emerald-600 dark:text-emerald-400 ml-2">{formatNumberRO(supplierTotal)} t</span>
                    </div>
                          ...
                          return (
                            <div key={cIdx}>
                              ...
                              <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1.5">
                                <div className="bg-emerald-500 h-1.5 rounded-full transition-all duration-300" style={{ width: `${codePercent}%` }} />
                              </div>
                            </div>
                          );
                        })}
                ...
                return (
                  <div key={idx} className="bg-gray-50 dark:bg-gray-900/30 rounded-lg p-3 border border-gray-200 dark:border-gray-700">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="text-base font-bold text-emerald-600 dark:text-emerald-400 ml-2">{formatNumberRO(operatorTotal)} t</span>
                    </div>
        ...
        <div className="px-4 py-3 border-b border-gray-200 dark:border-gray-700">
          <div className="flex items-center gap-2 flex-wrap">
            <h3 className="text-base font-semibold text-gray-900 dark:text-white whitespace-nowrap">
              Tichete Reciclare <span className="text-sm font-normal text-gray-500 dark:text-gray-400">{_totalLabel}</span>
            </h3>
            ...
            <div className="relative">
              ...
              <input type="text" value={_search} onChange={e => _setSearch(e.target.value)}
                ...
                className="h-8 pl-8 pr-3 w-[180px] text-xs bg-gray-50 dark:bg-gray-900 border border-gray-300 dark:border-gray-600 rounded-md text-gray-800 dark:text-gray-200 placeholder-gray-400 focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500 transition-colors" />
            </div>
            ...
            <select value={_recipientId} onChange={e => _setRecipientId(e.target.value)} className={selectClass}>
              <option value="">Toți reciclatorii</option>
            ...
            {canCreate && (
              <button onClick={onCreate} className="h-8 px-4 text-sm font-medium bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg transition-colors shadow-sm flex items-center gap-2 whitespace-nowrap">
          ...
          <table className="w-full">
            <thead className="bg-gray-50 dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700">
              <tr className="text-xs font-medium text-gray-700 dark:text-gray-300 uppercase tracking-wider">
                ...
                {/* ✅ Reciclator în header — Nr. Auto mutat în expand */}
                <th className={thClass}>Reciclator</th>
                ...
                _tickets.map((ticket) => (
                  <React.Fragment key={ticket.id}>
                    <tr className="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                      ...
                      {/* ✅ Reciclator în rând */}
                      <td className="px-4 py-3 text-sm text-gray-900 dark:text-white">{ticket.client_name || ticket.recipient_name || 'N/A'}</td>
