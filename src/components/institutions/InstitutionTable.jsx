...
/**
 * ============================================================================
 * INSTITUTION TABLE - FIXED WIDTH, NO SECTOR ICON
 * ============================================================================
 */
...
const InstitutionTable = ({
  ...
  const getTypeLabel = (type) => {
    return institutionTypes[type]?.label || type;
  };
        ...
        <table className="w-full table-fixed">
          <thead>
            <tr className="bg-gray-50 dark:bg-gray-900/50 border-b border-gray-200 dark:border-gray-700">
              <SortableHeader column="name" className="w-[300px]">Instituție</SortableHeader>
              <SortableHeader column="type" className="w-[130px]">Tip</SortableHeader>
              <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider w-[120px]">
                Sectoare
              </th>
              <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider w-[200px]">
                Contact
              </th>
              <th className="px-4 py-3 text-center text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider w-[90px]">
                Status
              </th>
          ...
          <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
            {institutions.map((inst) => (
              <tr key={inst.id} className="group hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                {/* Institution Name - Fixed width with truncate */}
                <td className="px-4 py-3 w-[300px]">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="text-sm font-medium text-gray-900 dark:text-white truncate" title={inst.name}>
                        {inst.name}
                      </p>
                      {inst.short_name && inst.short_name !== inst.name && (
                        <p className="text-xs text-gray-500 dark:text-gray-400 truncate" title={inst.short_name}>
                          {inst.short_name}
                        </p>
                      )}
                    </div>
                ...
                <td className="px-4 py-3 w-[130px]">
                  <span className={`inline-flex px-2.5 py-1 rounded-lg text-xs font-medium whitespace-nowrap ${getTypeBadgeClasses(inst.type)}`}>
                    {getTypeLabel(inst.type)}
                  </span>
                </td>

                {/* Sectors - No icons, just S1, S2, etc */}
                  ...
                  {inst.sectors && inst.sectors.length > 0 ? (
                    <div className="flex flex-wrap gap-1">
                      {inst.sectors.slice(0, 4).map(s => (
                        <span 
                          key={s.id} 
                          className="inline-flex px-1.5 py-0.5 bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400 text-xs font-medium rounded"
                        >
                          S{s.sector_number}
                        </span>
                      ))}
                  ...
                  <div className="space-y-1">
                    {(inst.email || inst.contact_email) && (
                      <div className="flex items-center gap-1.5 text-xs text-gray-600 dark:text-gray-400">
                        ...
                        <span className="truncate">{inst.email || inst.contact_email}</span>
                      </div>
                    )}
