...
import { FileText, Building2, MapPin, CheckCircle } from 'lucide-react';

const CONTRACT_TYPES = [
  { value: 'DISPOSAL', label: 'Depozitare', color: 'slate' },
  { value: 'WASTE_COLLECTOR', label: 'Colectare', color: 'emerald' },
  { value: 'SORTING', label: 'Sortare', color: 'violet' },
...
const ContractFilters = ({
  ...
  const filteredInstitutions = institutions.filter(inst => {
    switch (contractType) {
      ...
      case 'SORTING':
        return inst.type === 'SORTING_OPERATOR';
      case 'TMB':
  ...
  const getTypeButtonClass = (type, isActive) => {
    if (isActive) {
      const colorMap = {
        ...
        'SORTING': 'bg-violet-500 text-white shadow-violet-500/30',
          ...
          {CONTRACT_TYPES.map(type => (
            <button
              ...
              className={`
                ...
                ${getTypeButtonClass(type.value, contractType === type.value)}
              `}
            >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-2 border-t border-gray-200 dark:border-gray-700">
        ...
        <div>
          ...
          <select
            ...
            className="w-full px-3 py-2 bg-gray-50 dark:bg-gray-800 
